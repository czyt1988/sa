...
QwtPlotPropertySetDialog::QwtPlotPropertySetDialog(QWidget *parent,SAChart2D* plot)
    : QWidget(parent)
    ,m_plot(plot)
    ,m_enableSet(false)
{
...
void QwtPlotPropertySetDialog::addPlotSet(SAChart2D* plot)
{
    m_enableSet = false;
    //图表参数
...
void QwtPlotPropertySetDialog::addAxisSet(QtProperty* parentGroup, SAChart2D* plot,QwtPlot::Axis axis)
{
    m_enableSet = false;
    QtVariantProperty *pro = nullptr;
    ...
    pro->setValue(int(SAChart2D::hh_mm_ss));
...
void QwtPlotPropertySetDialog::addCurveSet(SAChart2D* plot)
{
...
void QwtPlotPropertySetDialog::setChartPtr(SAChart2D *chart)
{
...
void QwtPlotPropertySetDialog::setAxisScaleDrawStyle(QwtPlot::Axis axis, const QVariant& value)
{
    ...
    {
        SAChart2D::AxisDateScaleType st(SAChart2D::hh_mm_ss);
        if(property)
        {
            property->setEnabled(true);
            st = SAChart2D::AxisDateScaleType( static_cast<QtVariantProperty*>(property)->value().toInt() );
        }
        if(!dateScale)
        {//说明当前不是时间坐标轴
    ...
    if (id < ID_CurveIDBase)
    {
        switch(id)
        {
            ...
            case ID_XBottomDateScaleType:
                m_plot->setAxisDateFormat(QwtPlot::xBottom,SAChart2D::AxisDateScaleType(value.toInt()));
                return;
            case ID_XTopDateScaleType:
                m_plot->setAxisDateFormat(QwtPlot::xTop,SAChart2D::AxisDateScaleType(value.toInt()));
                return;
            case ID_YLeftDateScaleType:
                m_plot->setAxisDateFormat(QwtPlot::yLeft,SAChart2D::AxisDateScaleType(value.toInt()));
                return;
            case ID_YRightDateScaleType:
                m_plot->setAxisDateFormat(QwtPlot::yRight,SAChart2D::AxisDateScaleType(value.toInt()));
                return;
            case ID_XBottomInterval:
            case ID_XTopInterval:
            case ID_YLeftInterval:
