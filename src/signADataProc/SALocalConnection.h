...
private:
    ...
    ///
    Q_SIGNAL void callVectorPointFProcess(const QVector<QPointF>& points,quintptr widget,quintptr fig,quintptr item,QLocalSocket* socket);
private:
    void initThread();
private slots:
    //错误发生
    ...
    //接收到点数组的计算结果
    Q_SLOT void onProcessVectorPointFResult(SADataFeatureItem* result,quintptr widget,quintptr fig,quintptr item,QLocalSocket* socket);
