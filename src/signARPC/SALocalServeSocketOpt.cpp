...
void SALocalServeSocketOpt::deal(const SAProtocolHeader &head, const QByteArray &datas)
{
    ...
    if(0 == datas.size())
    {
        if(0 != head.dataCrc32)
        {
            qDebug() << "crc check invalid:(0 size) head is " << head
                     << " datas crc is :" << SACRC::crc32(datas) << "but datas size is 0";
            return;
        }
    }
    else
    {
        if(SACRC::crc32(datas) != head.dataCrc32)
        {
            qDebug() << "crc check invalid(not 0 size): head is " << head
                     << " datas size is :" << datas.size()
                     << " datas crc:" << SACRC::crc32(datas);
            return;
        }
    }
    ...
    case SA_LOCAL_SER_SHAKEHAND_CLASS:
        switch (funID) {
        case SA_LOCAL_SER_FUN_SHAKEHAND:
            //2-1
            dealHeartbeat(key);
            break;
        default:
    ...
    case SA_LOCAL_SER_PARAM_CLASS:
        switch (funID) {
        case SA_LOCAL_SER_FUN_STRING:
            dealString(datas,key);
            break;
        case SA_LOCAL_SER_FUN_POINT_SERIE_STATISTIC:
            //3-2
            deal2DPointFs(datas,key);
            break;
        case SA_LOCAL_SER_FUN_SEND_BIN:
            //3-99
            dealBin(datas,key);
            break;
        default:
...
void SALocalServeSocketOpt::sendHeartbeat(uint key)
{
    ...
    h.functionID = SA_LOCAL_SER_FUN_SHAKEHAND;
...
void SALocalServeSocketOpt::sendPointFSeries(const QVector<QPointF> &datas, uint key)
{
    ...
    h.functionID = SA_LOCAL_SER_FUN_POINT_SERIE_STATISTIC;
    h.tokenID = getToken();
    h.key = key;
...
void SALocalServeSocketOpt::sendString(const QString &str, uint key)
{
    ...
    h.type = SA_LOCAL_SER_PARAM_TYPE;
    h.classID = SA_LOCAL_SER_PARAM_CLASS;
    h.functionID = SA_LOCAL_SER_FUN_STRING;
    h.tokenID = getToken();
    h.key = key;
    QByteArray byteArray;
    QBuffer buffer(&byteArray);
...
void SALocalServeSocketOpt::sendErrorCode(const int errCode, uint key)
{
    ...
    h.functionID = SA_LOCAL_SER_FUN_OCCUR_ERROR;
    h.tokenID = getToken();
    h.key = key;
    QByteArray byteArray;
    QBuffer buffer(&byteArray);
...
void SALocalServeSocketOpt::sendBin(const QByteArray &bin, uint key)
{
    ...
    h.type = SA_LOCAL_SER_PARAM_TYPE;
    h.classID = SA_LOCAL_SER_PARAM_CLASS;
    h.functionID = SA_LOCAL_SER_FUN_SEND_BIN;
    h.tokenID = getToken();
    h.key = key;
    send(h,bin);
}
