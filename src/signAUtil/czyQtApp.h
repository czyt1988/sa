...
#include <QDataStream>
namespace SA{
        class SAPoint : public QPoint
		    {
			    public:
            SAPoint():QPoint(){}
            SAPoint(int xpos, int ypos):QPoint(xpos,ypos){}
			///
			/// \brief 重载 < 使得可用于Map
			/// \param a
			/// \param b
...
template<typename T>
QDataStream &operator<<(QDataStream & out, const SA::SAHashTable<T> & item);
template<typename T>
QDataStream &operator>>(QDataStream & in, SA::SAHashTable<T> & item);
...
QDataStream &operator>>(QDataStream & in, SA::SAHashTable<T> & item)
{
    typename SA::SAHashTable<T>::Table table;
    in >> table;
    item.setDatas(table);
    return in;
}
